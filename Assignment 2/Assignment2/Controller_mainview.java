...
public class Controller_mainview implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        if (Assignment2.class_variables.first_run == 1) {
            Assignment2.class_variables.employee_id_counter = Assignment2.class_variables.employee_id_counter + 1;
            generateEmployees();
            listview_employee.setItems(Assignment2.class_variables.employeeList);
            Assignment2.class_variables.first_run = 0;
        }
    }
    ...
    private void generateEmployees() {
        for(int counter=0;counter<11;counter++) {
            ...
            //Generate staff
            if(counter%2 == 0) {
                Assignment2.class_Staff newStaff = new class_Staff();
                newStaff.id = filler + counter;
                newStaff.name = "Staff " + counter;
                newStaff.type = "Staff";
                newStaff.accessLevel = 1;
                newStaff.hire();
                Assignment2.class_variables.employeeList.add(newStaff);
            } else {
                Assignment2.class_Faculty newFaculty = new class_Faculty();
                newFaculty.id = filler + counter;
                newFaculty.name = "Faculty " + counter;
                newFaculty.type = "Faculty";
                newFaculty.department = "History";
                newFaculty.hire();
                Assignment2.class_variables.employeeList.add(newFaculty);
            }
            Assignment2.class_variables.employee_id_counter = counter + 1;
        }
    }
    ...
    @FXML public void btn_delete_action(){
        ...
        if (listview_employee.getSelectionModel().getSelectedItem() == null) {
            ...
            Assignment2.class_variables.employeeList.remove(listview_employee.getSelectionModel().getSelectedItem());
